...
export function diagnosticoCompleto() {
  ...
  const emailsDuplicados = users
    .map((u: any) => u.email)
    .filter((email: string, index: number, arr: string[]) => arr.indexOf(email) !== index);
  ...
  const contratosSemNumero = contratos.filter((c: any) => !c.numero);
...
export function criarAdminAgora() {
  ...
  if (exists) {
    ...
    const admin = users.find((u: any) => u.email === adminUser.email);
...
export function listarContratos() {
  ...
  contratos.forEach((contrato: any, index: number) => {

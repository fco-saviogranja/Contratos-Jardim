...
if (
  ...
  console.error(
    "❌ ERRO CRÍTICO: Variáveis de ambiente SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY ou SUPABASE_ANON_KEY não configuradas!",
  );
...
app.post(
  "/make-server-1a8b02da/auth/setup-admin",
  async (c) => {
    try {
      console.log(
        "🔧 [SETUP] Iniciando configuração do administrador...",
      );
      const e = "controleinterno@jardim.ce.gov.br",
        p = "@Gustavo25";

      console.log("📋 [SETUP] Listando usuários existentes...");
      const { data: u } = await supabase.auth.admin.listUsers();
      console.log(
        `📊 [SETUP] Total de usuários no sistema: ${u?.users?.length || 0}`,
      );
      ...
      if (x) {
        console.log(
          `⚠️ [SETUP] Administrador já existe (ID: ${x.id}). Removendo usuário antigo...`,
        );
        const { error: delError } =
          await supabase.auth.admin.deleteUser(x.id);
        if (delError) {
          console.error(
            "❌ [SETUP] Erro ao deletar usuário antigo:",
            delError.message,
          );
        } else {
          console.log(
            "✅ [SETUP] Usuário antigo deletado com sucesso",
          );
        }
      ...
      console.log(
        "📝 [SETUP] Criando novo usuário no Supabase Auth...",
      );
      console.log(`   Email: ${e}`);
      console.log(`   Senha: ${p}`);

      const { data: d, error: err } =
      ...
      if (err) {
        console.error(
          "❌ [SETUP] Erro ao criar administrador no Supabase Auth:",
          err.message,
        );
        console.error(
          "❌ [SETUP] Detalhes do erro:",
      ...
      console.log(
        "✅ [SETUP] Usuário criado no Supabase Auth!",
      );
      console.log(`   ID: ${d.user.id}`);
      console.log(`   Email: ${d.user.email}`);
      console.log(
        `   Email confirmado: ${d.user.email_confirmed_at ? "Sim" : "Não"}`,
      );
      ...
      console.log(
        "🎉 [SETUP] Administrador configurado com sucesso!",
      );
      console.log("");
      ...
      console.error(
        "❌ [SETUP] Erro inesperado no setup do administrador:",
        error.message,
      );
...
app.post("/make-server-1a8b02da/auth/signup", async (c) => {
  try {
    const { email, password, nome, perfil, secretaria } =
      await c.req.json();
    ...
    if (u?.users?.find((u) => u.email === email)) {
      console.warn(
        `⚠️ [SIGNUP] E-mail já cadastrado: ${email}`,
      );
      return c.json(
        {
    ...
    if (e) {
      console.error(
        `❌ [SIGNUP] Erro ao criar usuário no Supabase Auth (${email}):`,
        e.message,
      );
      return c.json(
    ...
    console.log(
      `✅ [SIGNUP] Usuário criado com sucesso: ${email} (${perfil})`,
    );
    return c.json({
      success: true,
      user: { id: d.user.id, email, nome, perfil, secretaria },
    });
  } catch (error) {
    console.error(
      "❌ [SIGNUP] Erro inesperado ao criar usuário:",
      error.message,
    );
    return c.json(
  ...
  try {
    ...
    if (!authUser) {
      ...
      const solicitacoesPendentes =
        await kv.getByPrefix("solicitacao:");
      const solicitacaoPendente = solicitacoesPendentes.find(
        (s) => s.email === email && s.status === "pendente",
      );

      if (solicitacaoPendente) {
        console.warn(
          `⏳ [LOGIN] Solicitação pendente encontrada para: ${email}`,
        );
        return c.json(
          {
            error:
              "Sua solicitação de cadastro ainda está pendente de aprovação pelo administrador. Aguarde a análise.",
    ...
    if (error) {
      console.warn(
        `⚠️ [LOGIN] Erro ao autenticar ${email}:`,
        error.message,
      );
      ...
      if (error.message.includes("Email not confirmed")) {
        return c.json(
          {
            error:
              "E-mail não confirmado. Entre em contato com o administrador.",
          },
          401,
        );
      }
    ...
    if (!userData) {
      console.warn(
        `⚠️ [LOGIN] Usuário não encontrado no KV: ${authUser.id}, criando registro...`,
      );
      ...
      userData = {
        ...
        secretaria:
          authUser.user_metadata?.secretaria || "Não definida",
        situacao: "ativo",
        criadoEm: new Date().toISOString(),
    ...
    console.log(
      `✅ [LOGIN] Login bem-sucedido: ${email} (Perfil: ${userData.perfil})`,
    );
    return c.json({
      success: true,
    ...
    console.error(
      "❌ [LOGIN] Erro inesperado ao fazer login:",
      error.message,
    );
    return c.json(
...
app.post(
  "/make-server-1a8b02da/solicitar-cadastro",
  async (c) => {
    try {
      const {
        nome,
      ...
      if (
        ...
        console.warn(
          "⚠️ [SOLICITACAO] Campos obrigatórios faltando",
        );
        return c.json(
          { error: "Todos os campos são obrigatórios" },
          400,
      ...
      if (!isValidEmail(email)) {
        console.warn(
          `⚠️ [SOLICITACAO] E-mail inválido: ${email}`,
        );
        return c.json({ error: "E-mail inválido" }, 400);
      }
      ...
      if (
        s.find(
          (x) => x.email === email && x.status === "pendente",
        )
      ) {
        console.warn(
          `⚠️ [SOLICITACAO] Solicitação pendente já existe para: ${email}`,
        );
        return c.json(
          {
            error:
      ...
      if (u?.users?.find((x) => x.email === email)) {
        console.warn(
          `⚠️ [SOLICITACAO] E-mail já cadastrado: ${email}`,
        );
        return c.json(
          {
            error: "Este e-mail já está cadastrado no sistema",
      ...
      console.log(
        `✅ [SOLICITACAO] Solicitação criada: ${email} (${cargo})`,
      );
      return c.json(
        {
          success: true,
      ...
      console.error(
        "❌ [SOLICITACAO] Erro inesperado ao enviar solicitação:",
        error.message,
      );
      return c.json(
        {
...
app.get("/make-server-1a8b02da/solicitacoes", async (c) => {
  try {
    ...
    if (!u) {
      console.warn(
        "⚠️ [SOLICITACOES] Tentativa de acesso não autorizado",
      );
    ...
    const isAdmin =
      d &&
      (d.perfil === "admin" ||
        d.perfil === "Administrador CGM");

    if (!isAdmin) {
      console.warn(
        `⚠️ [SOLICITACOES] Acesso negado para usuário não-admin: ${u.email}`,
      );
      return c.json(
    ...
    const todasSolicitacoes =
      await kv.getByPrefix("solicitacao:");
    const solicitacoesPendentes = todasSolicitacoes.filter(
      (s) => s.status === "pendente",
    );
    solicitacoesPendentes.sort(
      (a, b) =>
    ...
    console.log(
      `✅ [SOLICITACOES] Lista de solicitações pendentes retornada (${solicitacoesPendentes.length} itens)`,
    );
    return c.json({
      success: true,
    ...
    console.error(
      "❌ [SOLICITACOES] Erro ao listar solicitações:",
      error.message,
    );
    return c.json(
      {
        error: `Erro ao listar solicitações: ${error.message}`,
      },
      500,
    );
  }
...
app.post(
  "/make-server-1a8b02da/solicitacoes/:id/aprovar",
  async (c) => {
    try {
      const u = await auth(c);
      ...
      if (!isAdmin) {
        console.warn(
          `⚠️ [APROVAR] Acesso negado para usuário não-admin: ${u.email}`,
        );
        return c.json(
          { error: "Acesso negado. Apenas administradores." },
          403,
      ...
      if (!s) {
        console.warn(
          `⚠️ [APROVAR] Solicitação não encontrada: ${id}`,
        );
        return c.json(
      ...
      if (s.status !== "pendente") {
        console.warn(
          `⚠️ [APROVAR] Solicitação já analisada: ${id}`,
        );
        return c.json(
      ...
      if (e) {
        console.error(
          `❌ [APROVAR] Erro ao criar usuário (${s.email}):`,
          e.message,
        );
        return c.json(
          { error: `Erro ao criar usuário: ${e.message}` },
      ...
      console.log(
        `✅ [APROVAR] Solicitação aprovada e usuário criado: ${s.email} (${perfil})`,
      );
      return c.json({
        success: true,
      ...
      console.error(
        "❌ [APROVAR] Erro inesperado ao aprovar solicitação:",
        error.message,
      );
      return c.json(
        {
          error: `Erro ao aprovar solicitação: ${error.message}`,
        },
        500,
      );
    }
...
app.post(
  "/make-server-1a8b02da/solicitacoes/:id/rejeitar",
  async (c) => {
    try {
      const u = await auth(c);
      if (!u) return c.json({ error: "Não autorizado" }, 401);
      ...
      if (!isAdmin) {
        console.warn(
          `⚠️ [REJEITAR] Acesso negado para usuário não-admin: ${u.email}`,
        );
        return c.json(
          { error: "Acesso negado. Apenas administradores." },
          403,
      ...
      if (!s) {
        console.warn(
          `⚠️ [REJEITAR] Solicitação não encontrada: ${id}`,
        );
        return c.json(
          { error: "Solicitação não encontrada" },
          404,
      ...
      if (s.status !== "pendente") {
        console.warn(
          `⚠️ [REJEITAR] Solicitação já analisada: ${id}`,
        );
        return c.json(
          { error: "Esta solicitação já foi analisada" },
          400,
      ...
      console.log(
        `✅ [REJEITAR] Solicitação rejeitada: ${s.email}`,
      );
      return c.json({
        success: true,
      ...
      console.error(
        "❌ [REJEITAR] Erro inesperado ao rejeitar solicitação:",
        error.message,
      );
      return c.json(
        {
          error: `Erro ao rejeitar solicitação: ${error.message}`,
        },
        500,
      );
    }
...
app.get(
  "/make-server-1a8b02da/debug/list-auth-users",
  async (c) => {
    try {
      console.log(
        "🔍 [DEBUG] Listando usuários do Supabase Auth...",
      );
      ...
      console.log(
        `✅ [DEBUG] Total de usuários: ${data.users.length}`,
      );
...
app.post("/make-server-1a8b02da/debug/reset-password", async (c) => {
  try {
    ...
    if (!user) {
      console.warn(
        `⚠️ [DEBUG] Usuário não encontrado: ${email}`,
      );
    ...
    if (error) {
      console.error(
        `❌ [DEBUG] Erro ao resetar senha: ${error.message}`,
      );
      return c.json(
    ...
    console.log(
      `✅ [DEBUG] Senha resetada com sucesso para: ${email}`,
    );
...
app.post("/make-server-1a8b02da/debug/check-user", async (c) => {
  try {
    ...
    console.log(
      `📊 [DEBUG] Auth: ${authUser ? "EXISTE" : "NÃO EXISTE"} | KV: ${kvUser ? "EXISTE" : "NÃO EXISTE"}`,
    );
...
app.post("/make-server-1a8b02da/debug/fix-user", async (c) => {
  try {
    ...
    if (!kvUser) {
      return c.json(
        { error: "Usuário não encontrado no KV Store" },
        404,
      );
    }
    ...
    if (authUser) {
      ...
      console.log(
        `✏️ [DEBUG] Usuário já existe no Auth, atualizando senha...`,
      );
      const { error } =
      ...
      // Não existe, criar
      console.log(
        `➕ [DEBUG] Criando usuário no Supabase Auth...`,
      );
      const { data, error } =
        await supabase.auth.admin.createUser({
          email: kvUser.email,
    ...
    console.error(
      "❌ [DEBUG] Erro ao corrigir usuário:",
      error.message,
    );
    return c.json({ error: `Erro: ${error.message}` }, 500);
  }
...
app.post("/make-server-1a8b02da/debug/fix-all-users", async (c) => {
  try {
    ...
    console.log(
      `📊 [DEBUG] Total de usuários no KV: ${allKvUsers.length}`,
    );
    ...
    console.log(
      `📊 [DEBUG] Total de usuários no Auth: ${authUsers.length}`,
    );
    ...
    console.log(
      `✅ [DEBUG] Processo concluído! ${fixedUsers.length} usuários corrigidos.`,
    );
    ...
    console.error(
      "❌ [DEBUG] Erro ao corrigir usuários:",
      error.message,
    );
    return c.json({ error: `Erro: ${error.message}` }, 500);
  }
...
app.post("/make-server-1a8b02da/debug/delete-user", async (c) => {
  try {
    ...
    if (deleteError) {
      console.error(
        `❌ [DEBUG] Erro ao excluir do Auth: ${deleteError.message}`,
      );
      return c.json(
        {
    ...
    try {
      ...
      console.warn(
        `⚠️ [DEBUG] Erro ao excluir do KV: ${kvError.message}`,
      );
    }

    console.log(
      `🎉 [DEBUG] Usuário ${userEmail} excluído com sucesso!`,
    );
...
app.post("/make-server-1a8b02da/debug/change-profile", async (c) => {
  try {
    ...
    if (!email || !novoPerfil) {
      return c.json(
        { error: "Email e novo perfil são obrigatórios" },
        400,
      );
    }

    console.log(
      `🔧 [DEBUG] Alterando perfil de: ${email} para: ${novoPerfil}`,
    );
    ...
    console.log(
      `✅ [DEBUG] Perfil alterado: ${oldProfile} → ${novoPerfil}`,
    );
...
app.post("/make-server-1a8b02da/debug/test-login", async (c) => {
  try {
    ...
    if (error) {
      console.warn(
        `⚠️ [DEBUG] Erro ao autenticar ${email}:`,
        error.message,
      );
      return c.json({
        success: false,
...
app.get("/make-server-1a8b02da/contratos", async (c) => {
  try {
    ...
    console.log(
      `✅ [CONTRATOS] Lista de contratos retornada (${d.length} itens)`,
    );
    return c.json({ success: true, contratos: d });
  } catch (error) {
    console.error(
      "❌ [CONTRATOS] Erro ao listar contratos:",
      error.message,
    );
    return c.json(
      { error: `Erro ao listar contratos: ${error.message}` },
      500,
    );
  }
});
...
app.get("/make-server-1a8b02da/contratos/:id", async (c) => {
  try {
    ...
    if (!d) {
      console.warn(
        `⚠️ [CONTRATOS] Contrato não encontrado: ${id}`,
      );
    ...
    console.error(
      "❌ [CONTRATOS] Erro ao buscar contrato:",
      error.message,
    );
    return c.json(
      { error: `Erro ao buscar contrato: ${error.message}` },
      500,
    );
  }
});
...
app.post("/make-server-1a8b02da/contratos", async (c) => {
  try {
    ...
    console.log(
      `✅ [CONTRATOS] Contrato criado: ${id} por ${u.email}`,
    );
    return c.json({ success: true, contrato: n }, 201);
  } catch (error) {
    console.error(
      "❌ [CONTRATOS] Erro ao criar contrato:",
      error.message,
    );
    return c.json(
...
app.put("/make-server-1a8b02da/contratos/:id", async (c) => {
  try {
    ...
    if (!e) {
      console.warn(
        `⚠️ [CONTRATOS] Contrato não encontrado para atualização: ${id}`,
      );
      return c.json({ error: "Contrato não encontrado" }, 404);
    }
    ...
    console.log(
      `✅ [CONTRATOS] Contrato atualizado: ${id} por ${u.email}`,
    );
    return c.json({ success: true, contrato: n });
  } catch (error) {
    console.error(
      "❌ [CONTRATOS] Erro ao atualizar contrato:",
      error.message,
    );
    return c.json(
      { error: `Erro ao atualizar contrato: ${error.message}` },
      500,
    );
  }
});
...
app.delete("/make-server-1a8b02da/contratos/:id", async (c) => {
  try {
    ...
    if (!contrato) {
      console.warn(
        `⚠️ [CONTRATOS] Contrato não encontrado para exclusão: ${id}`,
      );
      return c.json({ error: "Contrato não encontrado" }, 404);
    }
    ...
    console.log(
      `✅ [CONTRATOS] Contrato deletado: ${id} por ${u.email}`,
    );
    return c.json({
      success: true,
    ...
    console.error(
      "❌ [CONTRATOS] Erro ao deletar contrato:",
      error.message,
    );
    return c.json(
      { error: `Erro ao deletar contrato: ${error.message}` },
      500,
    );
  }
});
  ...
  try {
    ...
    const isAdmin =
      ...
      (userData.perfil === "admin" ||
        userData.perfil === "Administrador CGM");

    if (!isAdmin) {
      console.warn(
        `⚠️ [CONTRATOS] Tentativa de deletar todos os contratos por não-admin: ${u.email} (perfil: ${userData?.perfil})`,
      );
      return c.json(
        {
          error:
            "Acesso negado. Apenas administradores podem deletar todos os contratos.",
        },
        403,
      );
    }
    ...
    console.log(
      `🔥 [CONTRATOS] Iniciando deleção de todos os contratos por ${u.email}...`,
    );
    ...
    console.log(
      `📊 [CONTRATOS] ${contratos.length} contratos encontrados para deletar`,
    );

    if (contratos.length === 0) {
      console.log(
        "ℹ️ [CONTRATOS] Nenhum contrato encontrado para deletar",
      );
      return c.json({
        success: true,
    ...
    console.log(
      `🗑️ [CONTRATOS] Deletando contratos com IDs:`,
      ids,
    );
    ...
    console.log(
      `✅ [CONTRATOS] ${contratos.length} contratos deletados com sucesso por ${u.email}`,
    );
    return c.json({
      success: true,
    ...
    console.error(
      "❌ [CONTRATOS] Erro ao deletar todos os contratos:",
      error.message,
    );
    return c.json(
      { error: `Erro ao deletar contratos: ${error.message}` },
      500,
    );
  }
});
...
app.get("/make-server-1a8b02da/usuarios/me", async (c) => {
  try {
    ...
    if (!d) {
      console.log(
        `ℹ️ [USUARIOS] Usuário não encontrado no KV, usando dados do Auth: ${u.email}`,
      );
      return c.json({
        success: true,
        usuario: {
    ...
    console.log(
      `✅ [USUARIOS] Dados do usuário retornados: ${u.email}`,
    );
    return c.json({ success: true, usuario: d });
  } catch (error) {
    console.error(
      "❌ [USUARIOS] Erro ao buscar usuário atual:",
      error.message,
    );
    return c.json(
...
app.get("/make-server-1a8b02da/usuarios", async (c) => {
  try {
    ...
    console.log(
      `✅ [USUARIOS] Lista de usuários retornada (${d.length} itens)`,
    );
    return c.json({ success: true, usuarios: d });
  } catch (error) {
    console.error(
      "❌ [USUARIOS] Erro ao listar usuários:",
      error.message,
    );
    return c.json(
      { error: `Erro ao listar usuários: ${error.message}` },
...
app.put("/make-server-1a8b02da/usuarios/:id", async (c) => {
  try {
    ...
    if (!e) {
      console.warn(
        `⚠️ [USUARIOS] Usuário não encontrado para atualização: ${id}`,
      );
      return c.json({ error: "Usuário não encontrado" }, 404);
    }
    ...
    console.log(
      `✅ [USUARIOS] Usuário atualizado: ${id} por ${u.email}`,
    );
    return c.json({ success: true, usuario: n });
  } catch (error) {
    console.error(
      "❌ [USUARIOS] Erro ao atualizar usuário:",
      error.message,
    );
    return c.json(
...
app.delete("/make-server-1a8b02da/usuarios/:id", async (c) => {
  try {
    ...
    if (!e) {
      console.warn(
        `⚠️ [USUARIOS] Usuário não encontrado para exclusão: ${id}`,
      );
      return c.json({ error: "Usuário não encontrado" }, 404);
    }
    ...
    if (e.email === "controleinterno@jardim.ce.gov.br") {
      console.warn(
        `⚠️ [USUARIOS] Tentativa de excluir administrador principal`,
      );
      return c.json(
        {
          error:
    ...
    try {
      ...
      if (deleteError) {
        console.warn(
          `⚠️ [USUARIOS] Erro ao excluir usuário do Auth: ${deleteError.message}`,
        );
      } else {
        console.log(
          `✅ [USUARIOS] Usuário excluído do Supabase Auth: ${id}`,
        );
      }
    } catch (authError) {
      console.warn(
        `⚠️ [USUARIOS] Erro ao excluir do Auth (ignorado): ${authError.message}`,
      );
    }

    console.log(
      `✅ [USUARIOS] Usuário excluído: ${e.email} (${id}) por ${u.email}`,
    );
    return c.json({
      success: true,
    ...
    console.error(
      "❌ [USUARIOS] Erro ao excluir usuário:",
      error.message,
    );
    return c.json(
...
app.put("/make-server-1a8b02da/usuarios/me/perfil", async (c) => {
  try {
    ...
    if (!userData) {
      console.warn(
        `⚠️ [PERFIL] Usuário não encontrado: ${u.id}`,
      );
      return c.json({ error: "Usuário não encontrado" }, 404);
    }
    ...
    if (novaSenha) {
      ...
      if (loginError) {
        console.warn(
          `⚠️ [PERFIL] Senha atual incorreta para ${u.email}`,
        );
      ...
      if (updateError) {
        console.error(
          `❌ [PERFIL] Erro ao atualizar senha para ${u.email}:`,
          updateError.message,
        );
        return c.json(
          {
      ...
      console.log(
        `✅ [PERFIL] Senha atualizada para ${u.email}`,
      );
    }
    ...
    console.log(
      `✅ [PERFIL] Perfil atualizado para ${u.email}`,
    );
...
app.post("/make-server-1a8b02da/usuarios/me/foto", async (c) => {
  try {
    ...
    console.log(
      `✅ [FOTO] Foto de perfil atualizada para ${u.email}`,
    );
    return c.json({ success: true, fotoUrl });
  } catch (error) {
    console.error(
      "❌ [FOTO] Erro ao fazer upload da foto:",
      error.message,
    );
    return c.json(
...
app.get("/make-server-1a8b02da/secretarias", async (c) => {
  try {
    ...
    if (!s || s.length === 0) {
      console.log(
        "ℹ️ [SECRETARIAS] Inicializando secretarias padrão...",
      );
      const p = [
        {
          id: "1",
          nome: "Secretaria Municipal de Administração e Finanças",
          sigla: "SEMAF",
          responsavel: "",
          situacao: "ativa",
        },
        ...
        {
          id: "7",
          nome: "Secretaria Municipal de Esporte e Juventude",
          sigla: "SEMEJ",
          responsavel: "",
          situacao: "ativa",
        },
      ...
      console.log(
        "✅ [SECRETARIAS] Secretarias padrão inicializadas",
      );
    }

    console.log(
      `✅ [SECRETARIAS] Lista de secretarias retornada (${s.length} itens)`,
    );
    return c.json({ success: true, secretarias: s });
  } catch (error) {
    console.error(
      "❌ [SECRETARIAS] Erro ao listar secretarias:",
      error.message,
    );
    return c.json(
      { error: `Erro ao listar secretarias: ${error.message}` },
      500,
    );
  }
});
...
app.post("/make-server-1a8b02da/secretarias", async (c) => {
  try {
    ...
    console.log(
      `✅ [SECRETARIAS] Secretaria criada: ${sigla} por ${u.email}`,
    );
    return c.json({ success: true, secretaria: n });
  } catch (error) {
    console.error(
      "❌ [SECRETARIAS] Erro ao criar secretaria:",
      error.message,
    );
    return c.json(
      { error: `Erro ao criar secretaria: ${error.message}` },
      500,
    );
  }
});
...
app.put("/make-server-1a8b02da/secretarias/:id", async (c) => {
  try {
    ...
    if (!e) {
      console.warn(
        `⚠️ [SECRETARIAS] Secretaria não encontrada para atualização: ${id}`,
      );
      return c.json(
    ...
    console.log(
      `✅ [SECRETARIAS] Secretaria atualizada: ${id} por ${u.email}`,
    );
    return c.json({ success: true, secretaria: n });
  } catch (error) {
    console.error(
      "❌ [SECRETARIAS] Erro ao atualizar secretaria:",
      error.message,
    );
    return c.json(
      {
        error: `Erro ao atualizar secretaria: ${error.message}`,
      },
      500,
    );
  }
...
app.delete("/make-server-1a8b02da/secretarias/:id", async (c) => {
  try {
    ...
    if (!secretaria) {
      console.warn(
        `⚠️ [SECRETARIAS] Secretaria não encontrada para exclusão: ${id}`,
      );
      return c.json(
        { error: "Secretaria não encontrada" },
        404,
    ...
    console.log(
      `✅ [SECRETARIAS] Secretaria excluída: ${id} por ${u.email}`,
    );
    return c.json({
      success: true,
    ...
    console.error(
      "❌ [SECRETARIAS] Erro ao deletar secretaria:",
      error.message,
    );
    return c.json(
      { error: `Erro ao deletar secretaria: ${error.message}` },
      500,
    );
  }
});
...
app.get("/make-server-1a8b02da/alertas", async (c) => {
  try {
    ...
    console.log(
      `✅ [ALERTAS] Lista de alertas retornada (${d.length} itens)`,
    );
    return c.json({ success: true, alertas: d });
  } catch (error) {
    console.error(
      "❌ [ALERTAS] Erro ao listar alertas:",
      error.message,
    );
    return c.json(
      { error: `Erro ao listar alertas: ${error.message}` },
      500,
    );
  }
});
...
app.post("/make-server-1a8b02da/alertas", async (c) => {
  try {
    ...
    console.log(
      `✅ [ALERTAS] Alerta criado: ${id} por ${u.email}`,
    );
    return c.json({ success: true, alerta: n }, 201);
  } catch (error) {
    console.error(
      "❌ [ALERTAS] Erro ao criar alerta:",
      error.message,
    );
    return c.json(
      { error: `Erro ao criar alerta: ${error.message}` },
      500,
    );
  }
});
...
app.put("/make-server-1a8b02da/alertas/:id", async (c) => {
  try {
    ...
    if (!e) {
      console.warn(
        `⚠️ [ALERTAS] Alerta não encontrado para atualização: ${id}`,
      );
    ...
    console.log(
      `✅ [ALERTAS] Alerta atualizado: ${id} por ${u.email}`,
    );
    return c.json({ success: true, alerta: n });
  } catch (error) {
    console.error(
      "❌ [ALERTAS] Erro ao atualizar alerta:",
      error.message,
    );
    return c.json(
...
app.get("/make-server-1a8b02da/dashboard/stats", async (c) => {
  try {
    ...
    const st = {
      ...
      contratosAtivos: ct.filter((c) => c.status === "vigente")
        .length,
      contratosPendentes: ct.filter(
        (c) => c.status === "pendente",
      ).length,
      contratosVencidos: ct.filter(
        (c) => c.status === "vencido",
      ).length,
      totalAlertas: al.length,
      alertasCriticos: al.filter(
        (a) => a.prioridade === "critica",
      ).length,
      alertasNaoLidos: al.filter((a) => a.status === "nao_lido")
        .length,
      totalUsuarios: us.length,
      usuariosAtivos: us.filter((u) => u.situacao === "ativo")
        .length,
    };

    console.log(
      `✅ [DASHBOARD] Estatísticas retornadas: ${ct.length} contratos, ${us.length} usuários, ${al.length} alertas`,
    );
...
app.post("/make-server-1a8b02da/admin/limpar-usuarios", async (c) => {
  try {
    ...
    console.log(
      "📋 [PASSO 1] Listando usuários do Supabase Auth...",
    );
    ...
    console.log(
      `   Total de usuários encontrados: ${todosUsuarios.length}`,
    );
    ...
    console.log("");
    console.log(
      "🗑️ [PASSO 2] Excluindo usuários do Supabase Auth...",
    );

    for (const usuario of todosUsuarios) {
      if (usuario.email !== adminEmail) {
        console.log(
          `   Excluindo: ${usuario.email} (ID: ${usuario.id})`,
        );
    ...
    console.log("");
    console.log(
      "🗑️ [PASSO 3] Limpando KV Store de usuários...",
    );

    const kvUsuarios = await kv.getByPrefix("user:");
    console.log(
      `   Usuários no KV Store: ${kvUsuarios.length}`,
    );
    ...
    console.log("");
    console.log(
      "✅ [PASSO 4] Verificando usuário administrador...",
    );

    const adminUser = todosUsuarios.find(
    ...
    if (!adminUser) {
      ...
      if (!kvAdmin) {
        console.log(
          "⚠️ Admin não está no KV Store. Adicionando...",
        );
        ...
        if (kvAdmin.perfil !== "Administrador CGM") {
          console.log(
            "⚠️ Perfil do admin incorreto. Corrigindo...",
          );
    ...
    console.log("📊 RESUMO:");
    console.log(
      `   Usuários excluídos do Auth: ${usuariosExcluidos.length}`,
    );
    console.log(
      `   Usuários excluídos do KV: ${kvExcluidos.length}`,
    );
    console.log(
      `   Erros de exclusão: ${errosExclusao.length}`,
    );
    console.log("");
    ...
    console.error(
      "❌ [LIMPAR-USUARIOS] Erro inesperado:",
      error.message,
    );
...
app.post("/make-server-1a8b02da/reset-all-data", async (c) => {
  try {
    console.log(
      "🔥 [RESET] INICIANDO LIMPEZA TOTAL DO SISTEMA...",
    );
    console.log(
      "═══════════════════════════════════════════════════════════",
    );
    ...
    for (const contrato of contratos) {
      try {
        ...
        totalDeletados++;
        console.log(
          `   ✅ Contrato ${contrato.numeroContrato} excluído`,
        );
      } catch (error) {
        console.error(
          `   ❌ Erro ao excluir contrato ${contrato.numeroContrato}:`,
          error.message,
        );
        erros.push({
          tipo: "contrato",
    ...
    console.log(
      "👥 [RESET] Excluindo todos os usuários do KV Store...",
    );
    const usuarios = await kv.getByPrefix("user:");
    console.log(
      `   Total de usuários no KV: ${usuarios.length}`,
    );
    ...
    for (const usuario of usuarios) {
      try {
        ...
        totalDeletados++;
        console.log(
          `   ✅ Usuário ${usuario.email} excluído do KV`,
        );
      } catch (error) {
        console.error(
          `   ❌ Erro ao excluir usuário ${usuario.email}:`,
          error.message,
        );
        erros.push({
          tipo: "usuario_kv",
    ...
    console.log(
      "🔐 [RESET] Excluindo todos os usuários do Supabase Auth...",
    );
    ...
    for (const authUser of authUsers?.users || []) {
      try {
        ...
        totalDeletados++;
        console.log(
          `   ✅ Usuário ${authUser.email} excluído do Auth`,
        );
      } catch (error) {
        console.error(
          `   ❌ Erro ao excluir usuário ${authUser.email} do Auth:`,
          error.message,
        );
        erros.push({
          tipo: "usuario_auth",
    ...
    console.log(
      "📝 [RESET] Excluindo todas as solicitações...",
    );
    ...
    for (const solicitacao of solicitacoes) {
      try {
        ...
        totalDeletados++;
        console.log(
          `   ✅ Solicitação de ${solicitacao.email} excluída`,
        );
      } catch (error) {
        console.error(
          `   ❌ Erro ao excluir solicitação de ${solicitacao.email}:`,
          error.message,
        );
        erros.push({
          tipo: "solicitacao",
    ...
    console.log(
      "🔔 [RESET] Excluindo todas as notificações...",
    );
    ...
    console.log(
      `   Total de itens excluídos: ${totalDeletados}`,
    );
    ...
    return c.json({
      success: true,
      message:
        "Sistema completamente limpo! Todos os dados foram excluídos.",
      estatisticas: {
    ...
    console.error(
      "❌ [RESET] Erro crítico na limpeza total:",
      error.message,
    );
    console.error("❌ Stack trace:", error.stack);
    return c.json(
      { error: `Erro na limpeza total: ${error.message}` },
      500,
    );
  }
});
